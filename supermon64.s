...
NDX     = $0276             ; number of characters in keyboard buffer
KEYD    = $0277             ; keyboard buffer
BKVEC   = CBINV             ; BRK instruction vector (official name CBINV)
...
;MONSTOP    = $FFE1             ; check the MONSTOP key
GETIN   = MONRDKEY             ; get a character
...
STRT:    
        ...
        STX CHRPNT 
SMOVE:   
        JSR MONINPUT           ; CHRIN kernal call to input a character
...
; exit monitor [X]
EXITMON:    
        JMP RESTART         ; jump to warm-start vector to reinitialize BASIC
...
DSPM01:  
        ...
        BNE DSPM01
DSPBYT:  
        JSR MONSTOP            ; check for stop key
...
TMVAD:  
        ...
        BNE TINCOK
        JMP MONERROR           ; error if high byte overflowed
TDECAD:  
...
HNOFT:   
        JSR MONSTOP            ; no match, check for stop key
...
LERROR:  
        JMP MONERROR           ; if so, handle error
L8:      
...
LOADIT:  
        ...
        LDY #MSG6-MSGBAS    ; display "MONERROR" if checksum didn't match
        JSR SNDMSG
        JMP STRT            ; back to mainloop
LDADDR:  
...
ADDRMODE:.BYTE $40,$02,$45,$03   ; even opcodes
        .BYTE $D0,$08,$40,$09
        .BYTE $30,$22,$45,$33
        .BYTE $D0,$08,$40,$09
        .BYTE $40,$02,$45,$33
...
; addressing mode format definitions indexed by nybbles from ADDRMODE table
...
KADDR:  .WORD ASSEM-1,COMPAR-1,DISASS-1,FILL-1
        .WORD MONGOTO-1,HUNT-1,JSUB-1,DSPLYM-1
        .WORD DSPLYR-1,TRANS-1,EXITMON-1,DSTAT-1
...
SUPAD:  .WORD MONITOR           ; address of entry point